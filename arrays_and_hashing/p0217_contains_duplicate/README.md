...
- *What are the length bounds on `nums`?* — `0 <= len(nums) <= 10**5`; with fewer than two elements no duplicate can exist, which makes the answer `False`.
...
**Portability:** Values lie within `-10**9 <= nums[i] <= 10**9`, which sits well inside `int32` ($-2^{31}$ to $2^{31} - 1$). A C++ or Java port can therefore hold each element in a 32-bit `int` with no overflow surface. Python's `int` is arbitrary-precision, which removes the question from this implementation.
